...
// NewPagination initialize a new pagination calculation and returns a Paginater as result.
func NewPagination(total, pagingNum, current, numPages int) *Paginater {
	if pagingNum <= 0 {
		pagingNum = 1
	}
	if current <= 0 {
